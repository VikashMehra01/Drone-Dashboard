"""
Orthomosaic mapper service - decodes fed images, extracts DJI pose,
and drives MultiBandMap2D stitching.

Extracted and adapted from SkyForge's mapper_service.py (SkyForce/SkyForge-GCS-Repo/src/backend).
"""
...
from .satellite import SatelliteTileProvider

logger = logging.getLogger(__name__)

# Camera profile presets: { name: (focal_len_mm, sensor_width_mm, img_w, img_h) }
CAMERA_PROFILES = {
    "Skydroid C12": (24.0, 36.0, 1280, 720),
    "Custom": (24.0, 36.0, 1280, 720),
}
...
class MapperService:
    ...
    def __init__(self):
        # Skydroid C12 live stream parameters (approximate)
        self.focal_len_mm = 24.0
        self.sensor_width_mm = 36.0
        self.img_width_px = 1280
        self.img_height_px = 720
        self.update_parameters(self.focal_len_mm, self.sensor_width_mm, self.img_width_px, self.img_height_px)
        ...
        self.pg_w_gps = 9.0
        self.pg_w_match = 0.25
        self.pg_min_matches = 15
        self.pg_iou_threshold = 0.10
        self.pg_solve_every = 5
        self.pose_optimizer = PoseGraphOptimizer(
            w_gps=self.pg_w_gps, w_match=self.pg_w_match,
            min_matches=self.pg_min_matches,
            iou_threshold=self.pg_iou_threshold,
            solve_every=self.pg_solve_every,
        )
        ...
        self._last_frame_pose_xy = None

        # Satellite underlay
        self.satellite_provider = None
        self.show_satellite = True
        self.satellite_area = 500  # metres

    def update_parameters(self, focal_len_mm, sensor_width_mm, img_width_px, img_height_px=None):
        """Update camera intrinsic parameters."""
        self.focal_len_mm = focal_len_mm
        self.sensor_width_mm = sensor_width_mm
        self.img_width_px = img_width_px
        self.img_height_px = img_height_px or int(img_width_px * 0.75)
        fx = img_width_px * (focal_len_mm / sensor_width_mm)
        fy = fx
        cx = img_width_px / 2.0
        cy = self.img_height_px / 2.0
    ...
    def _camera_matrix_for_frame(self, frame_w, frame_h):
        """Build intrinsics for the actual frame size using the selected profile FOV."""
        fx = frame_w * (self.focal_len_mm / self.sensor_width_mm)
        fy = fx
        cx = frame_w / 2.0
        cy = frame_h / 2.0
        return np.array(
    ...
    MAX_IMG_DIM = 2000
    MIN_FRAME_STD = 6.0
    MIN_FRAME_SATURATION = 8.0
    MIN_FRAME_SHARPNESS = 18.0

    def _is_invalid_video_frame(self, frame):
        ...
        try:
            ...
            return (
                (float(gray.std()) < self.MIN_FRAME_STD
                 and float(hsv[:, :, 1].mean()) < self.MIN_FRAME_SATURATION)
                or sharpness < self.MIN_FRAME_SHARPNESS
            )
        except Exception:
            return False
    ...
    def _frame_signature_diff(self, a, b):
        if a is None or b is None:
            return 999.0
        return float(cv2.absdiff(a, b).mean())

    def _frame_motion_px(self, a, b):
        if a is None or b is None:
            return 0.0
        try:
            shift, response = cv2.phaseCorrelate(np.float32(a), np.float32(b))
            if response < 0.05:
                return 0.0
    ...
    def _pose_xy_distance(self, pose):
        xy = np.array([pose[0, 3], pose[1, 3]], dtype=float)
        if self._last_frame_pose_xy is None:
            return 0.0, xy
        return float(np.linalg.norm(xy - self._last_frame_pose_xy)), xy
    ...
    def process_image(self, img_bytes):
        ...
        # Initialize satellite tile provider on first successful GPS fix
        if self.satellite_provider is None and self.show_satellite:
            lat = self.pose_extractor.origin_lat
            lon = self.pose_extractor.origin_lon
            if lat is not None and lon is not None:
                try:
                    self.satellite_provider = SatelliteTileProvider(
                        center_lat=lat, center_lon=lon,
                        area_meters=self.satellite_area,
                    )
                    self.satellite_provider.build()
                except Exception as e:
                    logger.error(f"[Satellite] Init failed: {e}")
                    self.satellite_provider = None
        ...
        if self._is_invalid_video_frame(frame):
            return {"status": "skipped", "message": "Skipped blank/gray RTSP frame"}
        sig = self._frame_signature(frame)
        sig_diff = self._frame_signature_diff(self._last_frame_sig, sig)
        img_motion_px = self._frame_motion_px(self._last_frame_sig, sig)
        pose_move_m, pose_xy = self._pose_xy_distance(pose_matrix)
        if sig_diff < 1.5 and pose_move_m > 1.0:
            return {
                "status": "skipped",
                "message": f"Skipped frozen video frame ({pose_move_m:.1f} m pose move)",
            }
        if img_motion_px < 1.0 and pose_move_m > 1.0:
            return {
                "status": "skipped",
                "message": f"Skipped buffered video frame ({pose_move_m:.1f} m pose move)",
            }
        if img_motion_px > 2.0 and pose_move_m < 0.25:
            return {
                "status": "skipped",
                "message": f"Skipped stale telemetry frame ({img_motion_px:.1f} px image move)",
            }
        
        # Downscale large images to prevent timeout on hosted environments
        h, w = frame.shape[:2]
        perf['image_resolution'] = f"{w}x{h}"
        K_frame = self._camera_matrix_for_frame(w, h)
        ...
        self._last_frame_pose_xy = pose_xy
    ...
    def _get_satellite_bg(self, mapper):
        """Build satellite background canvas sized for the given mapper's tiles."""
        if self.satellite_provider is None or not self.show_satellite:
            return None
        try:
            tile_keys = list(mapper.tiles.keys())
            return self.satellite_provider.get_background_for_render(
                tile_keys, mapper.tile_size, mapper.resolution,
            )
        except Exception as e:
            logger.error(f"[Satellite] BG error: {e}")
            return None
    ...
    def get_map_image(self, which='main'):
        ...
        if which == 'raw' and self.mapper_raw is not None:
            sat_bg = self._get_satellite_bg(self.mapper_raw)
            img = self.mapper_raw.render_map(0, show_flight_path=show_fp, satellite_bg=sat_bg)
        elif which == 'optimized' and self.mapper_optimized is not None:
            sat_bg = self._get_satellite_bg(self.mapper_optimized)
            img = self.mapper_optimized.render_map(0, show_flight_path=show_fp, satellite_bg=sat_bg)
        else:
            sat_bg = self._get_satellite_bg(self.mapper)
            img = self.mapper.render_map(0, show_flight_path=show_fp, satellite_bg=sat_bg)
